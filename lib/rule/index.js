...
ruleClass.prototype._cb_applyRule = function (callback , msg, _meta) {
    ...
    if (nextRuleObj) {
        return self._applyRule(self._cb_applyRule.bind(self, callback, msg, _meta), msg, nextRuleObj, _meta);
    }
...
ruleClass.prototype._applyAction = function  (callback, msg, action) {
...
ruleClass.prototype._cb_applyAction = function (callback, msg, _meta) {
    ...
    if (!nextAction || _.get(msg, ACTION.R_ACTIONS.RE_EXIT, false) === true) {
        ...
        return self._applyRule(callback, msg, nextRule, _meta);

    } else {
        // Execute remaining actions

        return self._applyAction(self._cb_applyAction.bind(self, callback, msg, _meta), msg, nextAction.action);
    }
...
ruleClass.prototype._checkDecision = function (callback, msg, rule ,_meta,
    ...
    case 'rule':
        ...
        self._applyRule(callback, msg, ruleObj , _meta);
        break;
    case 'condition':
        // Unable to reach final decision ,
        // execute more conditions ...
        self._applyCondition(self._cb_applyCondition.bind(self, callback, msg, rule,
                                                             _meta, nextConditionObj,
                                                             finalDecision), msg, nextConditionObj);
        break;
    case 'action':
        // Conditions were met ,
        // execute actions now ....
        self._applyAction(self._cb_applyAction.bind(self, callback, msg, _meta), msg, actionObj.action);
        break;
...
ruleClass.prototype._applyRule = function (callback, msg, ruleObject, _meta) {
    ...
    // Refresh indices when a new rule is applied

    _.set(self.flowControl, msg._meta.id + ".rule_condition_idx", 0);
    _.set(self.flowControl, msg._meta.id + ".rule_action_idx", 0);
...
ruleClass.prototype.executeRules = function(callback, msg, tag) {
    ...
    */
    ...
    return self._applyRule(self._cb_applyRule.bind(self, callback, msg, _meta), msg, currentRuleObj, _meta);
