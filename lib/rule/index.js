...
ruleClass.prototype._cb_applyRule = function (msg, currentRuleObject) {
    ...
    if (ruleObj) {
        ...
        return self._applyRule(self._cb_applyRule.bind(self, msg, ruleObj), msg, ruleObj.rule);
    }
...
ruleClass.prototype._cb_applyCondition = function (msg, conditionObj, rule, cDecision) {
    ...
    _.set(self.flowControl, msgId + ".rules." + rule.id + ".conditions." + conditionObj.condition.id, cDecision);
...
ruleClass.prototype._checkDecision = function (msg, rule) {
    ...
    _.keys(ruleConditions).forEach(function (conditionId, idx) {
        ...
        if (self.R_OPERATORS.AND === rOperator) {
            ...
            // Have a try catch for custom conditions ... this should not execute
            // unless all conditions have been arrived.
...
ruleClass.prototype._applyRule = function (callback, msg, rule) {
    ...
    if (_.get(msg, ACTION.R_ACTIONS.RE_EXIT, false) === true) {
        ...
        // stop execution , pass control _cb_applyRule to execute next rule.
        return callback();
    }
    ...
    case true :
        ...
        return self._applyAction(self._cb_applyAction.bind(self, msg, actionObj, rule), msg, actionObj);
    ...
    case null:
        ...
        return self._applyCondition(self._cb_applyCondition.bind(self, msg, conditionObj, rule) , msg, conditionObj);
    default:
        // don't know how to handle this for now ....
        console.log("YKW .... The final decision is " , finalDecision);
...
ruleClass.prototype.executeRules = function(callback, msg, tag) {
    ...
    */
    ...
    // This is also being set from cb_applyRule , have to move this to a common function ....

    _.set(self.flowControl, msgId + ".rules." + currentRuleObj.rule.id + ".conditions", {});

    _.set(self.flowControl, msgId + ".rules." + currentRuleObj.rule.id + ".total_conditions", currentRuleObj.rule.conditions.length);

    _.set(self.flowControl, msgId + ".rules." + currentRuleObj.rule.id + ".actions", {});

    _.set(self.flowControl, msgId + ".rules." + currentRuleObj.rule.id + ".total_actions", currentRuleObj.rule.actions.length);
    ...
    return self._applyRule(self._cb_applyRule.bind(self, msg, currentRuleObj), msg, currentRuleObj.rule);
